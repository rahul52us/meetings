...
const EditJobModal: React.FC<AddJobModalProps> = ({
  isOpen,
  onClose,
  fetchRecords,
  jobScope,
  ...
  useEffect(() => {
    setFormValues(() => ({
      ...data.jobDetails,
      employerName: {
        label: data?.employerName?.name,
        value: data?.employerName?._id,
      },
      agentName: {
        label: data?.agentName?.name,
        value: data?.agentName?._id,
      },
      skills: Array.isArray(data.jobDetails?.skills)
        ? data.jobDetails.skills.filter(Boolean).join(', ')
        : typeof data.jobDetails?.skills === 'string'
      ...
      <KeyboardAvoidingView
        ...
        <View style={styles.stickyHeader}>
          ...
          <Pressable
            ...
            accessibilityLabel="Close Edit job modal"
