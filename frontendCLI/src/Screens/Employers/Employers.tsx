    ...
    async (page: number, value: string) => {
      ...
      try {
        ...
        if (response.status === 'success' && Array.isArray(response.data)) {
          ...
          toastNotify({
            status: 'error',
            title: response.message || 'Failed to fetch employers',
          });
        }
      } catch (error: any) {
        ...
        toastNotify({
          status: 'error',
          title: error.message || 'Failed to fetch employers',
        });
      } finally {
        setLoading(false);
        setRefreshing(false);
          ...
          >
            ...
            <View style={[styles.modalBody, { alignItems: 'center', gap: 16 }]}>
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <Text style={styles.buttonText}>👤 Add Single Employer</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                accessibilityLabel="Add bulk employers"
                accessibilityRole="button"
                activeOpacity={0.85}
              >
                <Text style={styles.buttonText}>
                  📁 {bulkUploadLoading ? 'Uploading...' : 'Add Bulk Employers'}
                </Text>
              </TouchableOpacity>
            </View>
        ...
        {loading && (
          <View style={styles.loaderOverlay}>
            ...
            <Text style={styles.loadingText}>
              {initialLoad ? 'Loading Employers...' : 'Searching Employers...'}
            </Text>
          </View>
        )}
                ...
                ListEmptyComponent={
                  loading ? null : (
                    <View style={styles.emptyState}>
                      <Text style={styles.noDataText}>No Employers found</Text>
                      <Pressable
                        style={styles.button}
                        onPress={handleRefresh}
                        accessibilityLabel="Retry loading Employers"
                        accessibilityRole="button"
                      >
                        <Text style={styles.buttonText}>Retry</Text>
                      </Pressable>
