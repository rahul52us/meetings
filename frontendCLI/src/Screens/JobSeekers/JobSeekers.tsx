...
const JobSeekers: React.FC = () => {
    ...
    async (page: number, value: string) => {
      ...
      try {
        ...
        if (response.status === 'success' && Array.isArray(response.data)) {
          ...
          toastNotify({
            status: 'error',
            title: response.message || 'Failed to fetch job seekers',
          });
        }
      } catch (error: any) {
        ...
        toastNotify({
          status: 'error',
          title: error.message || 'Failed to fetch job seekers',
        });
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const renderTableRow = useCallback(
    ({ item, index }: { item: any; index: number }) => (
      <Pressable
        ...
        accessibilityRole="button"
      >
        <Text style={[styles.tableCell, { width: COLUMN_WIDTHS.profileId , textAlign : "center"}]} numberOfLines={1} ellipsizeMode="tail">
          {item?.profileId || 'N/A'}
        </Text>
        <Text style={[styles.tableCell, { width: COLUMN_WIDTHS.name , textAlign : "center"}]} numberOfLines={1} ellipsizeMode="tail">
          {item.accountDetails?.name || item.profileDetails?.name || 'N/A'}
        </Text>
        <Text style={[styles.tableCell, { width: COLUMN_WIDTHS.email, textAlign : "center" }]} numberOfLines={1} ellipsizeMode="tail">
          {item.accountDetails?.email || 'N/A'}
        </Text>
        <Text style={[styles.tableCell, { width: COLUMN_WIDTHS.username }]} numberOfLines={1} ellipsizeMode="tail">
          {item.moNumber || item.accountDetails?.username || 'N/A'}
        </Text>
        <Pressable
          style={[styles.tableCell, { width: COLUMN_WIDTHS.aadharNumber }]}
          onPress={() => {
            if (!item.accountDetails?.is_active) {
              setValidationItem({ ...item, user: item?.user });
        ...
        >
          <Text style={styles.tableCellText} numberOfLines={1} ellipsizeMode="tail">
            {maskAadhaar(item?.profileDetails?.aadharNumber)}
          </Text>
        </Pressable>
        <Text
  style={[
    styles.tableCell,
    {
      ...
      textAlign: 'center',
      color: item?.accountDetails?.is_active === true
        ? '#16a34a' // green for verified
        : item?.accountDetails?.is_active === false
        ? '#dc2626' // red for not verified
  ...
  ellipsizeMode="tail"
>
  {item?.accountDetails?.is_active === true
    ? 'Verified'
    : item?.accountDetails?.is_active === false
    ? 'Not Verified'
    : '—'}
          ...
          >
            ...
            <View style={[styles.modalBody, { alignItems: 'center', gap: 16 }]}>
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <Text style={styles.buttonText}>👤 Add Single Job Seeker</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                accessibilityLabel="Add bulk job seekers"
                accessibilityRole="button"
                activeOpacity={0.85}
              >
                <Text style={styles.buttonText}>
                  📁 {bulkUploadLoading ? 'Uploading...' : 'Add Bulk Job Seekers'}
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <View style={styles.headerActions}>
            <View style={styles.searchContainer}>
              <TextInput
                ...
                accessibilityLabel="Search Job Seekers"
        ...
        {loading && (
          <View style={styles.loaderOverlay}>
            ...
            <Text style={styles.loadingText}>
              {initialLoad ? 'Loading Job Seekers...' : 'Searching Job Seekers...'}
            </Text>
          </View>
        )}
                ...
                ListEmptyComponent={
                  loading ? null : (
                    <View style={styles.emptyState}>
                      <Text style={styles.noDataText}>No Job Seekers found</Text>
                      <Pressable
                        style={styles.button}
                        onPress={handleRefresh}
                        accessibilityLabel="Retry loading Job Seekers"
                        accessibilityRole="button"
                      >
                        <Text style={styles.buttonText}>Retry</Text>
                      </Pressable>
